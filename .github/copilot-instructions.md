...
stockmarketwords.com is a **Hugo static site** (`hugo/site/`) fed by a
**Python CLI** (`python3/`, entry point `ticker-cli`) that runs after each
US trading day, persisting state in a **SQLite database**
(`data/market_data.db`) and emitting JSON build artifacts that Hugo consumes.
The site is **multilingual** (English + Simplified Chinese) using Hugo's
built-in i18n.

Foundation docs — read at least the architecture overview before making any
non-trivial change:
...
6. [`docs/design/20260408_013203_UTC_deployment.md`](../docs/design/20260408_013203_UTC_deployment.md) — how a change in `main` becomes a live deploy.

**Spec-driven development:** feature work starts with a spec under
[`docs/specs/`](../docs/specs/README.md). Before writing code for a new
feature, write or read the spec. Active specs:
