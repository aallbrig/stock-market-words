...
""".format(datetime.now().isoformat(), datetime.now().strftime("%Y-%m-%d %H:%M:%S UTC"))
    ...
    logger.info("ℹ️  Strategy pages (strategy-*.md) are manually maintained")
    ...
    for strategy_key, strategy_info in strategies.items():
        ...
        for row in rows:
            ticker_data = {
                ...
                'industry': row[4] if row[4] else None,
...
def generate_all_tickers_json(dry_run=False):
    """
    Generate hugo/site/data/all_tickers.json — the merged data file consumed by the
    ...
    if dry_run:
        ...
        logger.info(f"DRY RUN: Output to {HUGO_DATA_DIR}/all_tickers.json")
        return

    logger.info("=== Generating all_tickers.json ===")
    ensure_hugo_dirs()
    ...
    for row in rows:
        ...
        tickers.append({
            ...
            'week52Low': safe_float(row[17], 2),
            'scores': {
                'dividendDaddy': int(row[18]) if row[18] is not None else None,
                'moonShot': int(row[19]) if row[19] is not None else None,
    ...
    output = {
        '_meta': {
            'auto_generated': True,
            'generated_by': 'python3/src/stock_ticker/hugo_generators.py :: generate_all_tickers_json()',
            'generated_at': datetime.now().isoformat(),
    ...
    output_path = HUGO_DATA_DIR / "all_tickers.json"
    with open(output_path, 'w') as f:
        json.dump(output, f, separators=(',', ':'))

    logger.info(f"✓ all_tickers.json: {len(tickers)} tickers → {output_path}")


def generate_ticker_lookup_json(dry_run=False):
    """
    Generate hugo/site/static/data/ticker-lookup.json — a minimal symbol+name
    ...
    if dry_run:
        ...
        logger.info(f"DRY RUN: Output to {HUGO_STATIC_DATA_DIR}/ticker-lookup.json")
        return

    logger.info("=== Generating ticker-lookup.json ===")
    ensure_hugo_dirs()
    ...
    tickers = [{"s": row[0], "n": row[1] or ""} for row in rows]

    data = {
        "_meta": {
            "auto_generated": True,
            "generated_by": "python3/src/stock_ticker/hugo_generators.py :: generate_ticker_lookup_json()",
            "generated_at": datetime.now().isoformat(),
            "warning": "DO NOT EDIT - This file is automatically generated and will be overwritten",
        },
        "total": len(tickers),
    ...
    static_path = HUGO_STATIC_DATA_DIR / "ticker-lookup.json"
    with open(static_path, "w") as f:
...
def generate_all_hugo_content(dry_run=False):
    ...
    if dry_run:
        ...
        logger.info("DRY RUN: - all_tickers.json (ticker pages data)")
        logger.info("DRY RUN: - ticker-lookup.json (autocomplete data)")
        return
    ...
    # Generate all_tickers.json for individual ticker pages
    logger.info("Step 5: Generating all_tickers.json for ticker pages...")
    generate_all_tickers_json(dry_run=False)
    logger.info("")

    # Generate ticker-lookup.json for client-side autocomplete
    logger.info("Step 6: Generating ticker-lookup.json for lookup autocomplete...")
